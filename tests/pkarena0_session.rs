...
/// Hands in the fixture that record an action order the engine no longer
/// accepts, and why.
///
/// `pkarena0-hand-002` is a captured `pkarena0` session, so it is a recording of
/// whatever the engine did at the time — not an independently authored
...
/// ```
///
/// Seat 8 was in for 350 and owed 1983 when seat 5 raised to 2333. Clockwise
/// from seat 5 the next live seat is 8, not 4. Seat 4 acted out of turn, and
/// seat 8 was then given a decision it should have made two actions earlier.
///
/// That is exactly `DEFECT_022`: `next_to_act` restarted its scan under the gun
/// (seat 4) instead of clockwise of the raiser (seat 5). The recording is
/// independent corroboration of the defect from outside the Pluribus corpus —
/// and it is a golden file generated by the code it was meant to guard.
///
/// It cannot be regenerated: the bot decisions in it are a captured session, not
/// a reproducible script. It is skipped rather than corrected, because editing a
/// recording to say what we now believe should have happened would destroy the
/// only evidence of what did.
const REPLAY_SKIP: &[&str] = &["pkarena0-hand-002"];
...
fn all_hands_replay_consistently() {
    ...
    for (idx, result) in collection.replay_all().into_iter().enumerate() {
        if REPLAY_SKIP.contains(&collection.hands()[idx].hand.id.as_str()) {
            continue;
        }
        if let Err(e) = &result {
            failures.push(format!("{}: {e}", collection.hands()[idx].hand.id));
            continue;
        }
