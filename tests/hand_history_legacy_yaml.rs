...
const PKARENA_SESSION_YAML: &str =
...
fn legacy_collection_round_trips_without_player_id() {
    let original = HandCollection::from_yaml(PKARENA_SESSION_YAML)
        .expect("legacy session YAML should parse");
    ...
    let re_serialized = original
        .to_yaml()
        .expect("collection should re-serialize");
    assert!(
        !re_serialized.contains("player_id"),
        "round-tripped YAML must not introduce a player_id field"
    );

    let reparsed = HandCollection::from_yaml(&re_serialized)
        .expect("re-serialized YAML should parse");
    assert_eq!(
        original, reparsed,
        "round-tripped HandCollection should equal original"
    );
}

#[test]
fn legacy_hand_history_round_trips_without_player_id() {
    let original = HandHistory::from_yaml(THE_HAND_YAML)
        .expect("legacy hand YAML should parse");
    ...
    let re_serialized = original
        .to_yaml()
        .expect("hand history should re-serialize");
    assert!(!re_serialized.contains("player_id"));

    let reparsed = HandHistory::from_yaml(&re_serialized)
        .expect("re-serialized YAML should parse");
