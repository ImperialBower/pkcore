...
- [ ] 🤖 **`TableCelled::act_raise` underflows on a short all-in** — the pre-validation guard is skipped when `would_be_all_in` is true, which is exactly the case where `amount < self.bet` (a short stack shoving under a bigger raise). `amount - self.bet.get()` then underflows: panic in debug, wrap to a huge `usize` in release, corrupting `raise_increment` and every subsequent `min_raise()` on that street. The sibling `Table::act_raise` guards the identical case with `amount.saturating_sub(self.bet)` and a comment explaining why — the two table implementations have drifted. No test exercises `act_raise` with a short all-in. Suggested: use `saturating_sub`, matching the sibling. (`src/casino/table_celled.rs:600`, cf. `src/casino/table/actions.rs:653`)

- [ ] 🤖 **`SolverCache::cache_key` omits `max_iterations` and `cfr_variant`** — both are public `SolverConfig` fields that materially change the returned `SolverResult` (iteration count, exploitability, equilibrium quality), but neither is hashed. Two configs differing only in iteration count or CFR variant collide on the same cache file, so a request for a long DCFR solve can be served a short vanilla-CFR result that looks valid. Suggested: hash `max_iterations` and a `Hash`-derived encoding of `cfr_variant` into the key. (`src/analysis/gto/solver_cache.rs:97`, fields at `src/analysis/gto/solver_config.rs:402`, `:407`)

- [ ] 🤖 **`OmahaHigh::eval` does not enforce Omaha's exactly-2-hole-cards rule** — it picks 2 hole cards, then hands 7 cards to the unconstrained best-5-of-7 evaluator, which is free to ignore both and play the board. Illegal in Omaha. The sibling `OmahaHigh::permutations` (`:71`) does it correctly (2 hole + 3 board). **Compounding the risk:** the deprecated `Four::omaha_high` carries a doc comment calling this exact pattern *"a serious flaw"* and directs readers to `OmahaHigh::eval()` as *"the valid, tested logic"* — it is neither valid nor tested (zero test calls in the file), and it is exported via `prelude`. Not on the live showdown path, which correctly uses `permutations()`. Suggested: reimplement `eval()` over `permutations()`, or deprecate it as its sibling was, and fix the misleading comment either way. (`src/games/omaha.rs:38`, comment at `src/arrays/four.rs:63`)
