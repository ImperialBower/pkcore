...
| Tree serialization | ✅ `SolverResult` serializes via `to_binary_bytes` / `save_json`; board+range-hash cache in `solver_cache.rs` ([EPIC-15a](EPIC-15a_GTO_Solver_Cache-CLOSED.md)) |
...
9. ✅ **Serialization** — `SolverResult` save/load done; board+range-hash cache
   complete in `solver_cache.rs` ([EPIC-15a](EPIC-15a_GTO_Solver_Cache-CLOSED.md))
