...
3. **Widen the `Pile` combinatorics signatures** to `impl Iterator<Item = Vec<Card>>` (`src/lib.rs:861,866`), and mirror on `Cards`/`Deck`/`Outs`. RPITIT is available and `Pile` is never used as `dyn Pile`. Semver-breaking, but 13 internal call sites and **zero** across the 15 pkcore consumers. Moves checklist item 4 toward *pass*; does **not** reach coupling 5 on its own — `uuid` and `wincounter` would have to go too, and they are recorded as deliberate.
