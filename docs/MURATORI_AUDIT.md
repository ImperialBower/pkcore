...
- **Why it does not move the score: the round-trip is hand-shaped, not state-shaped.** Sketch 2 measures both walls. `serde_json::to_string(&session.table)` **fails to compile** — `the trait bound Table: serde::Serialize is not satisfied`. `Table` derives only `Clone, Debug, Eq, PartialEq` (`table.rs:86`), and neither do `Seats` (`table/seats.rs:25`), `Seat` (`table/seat.rs:22`), `Player` (`table/player.rs:23`), or `Cards` (`src/cards.rs:35`). And the Pluribus path is not a substitute: run against a mid-hand table, `Pluribus::try_from(&session.table)` returns `Err` (Sketch 2, printed `mid-hand export: false`), because the export harvests a *finished* `event_log`. Even on success the inverse forces `Pluribus::STARTING_STACK` into every seat and the button onto the last seat (`nubibus.rs:422,453`) — right for replaying the experiment's corpus, wrong for resuming a live cash table.
...
| 6 | Data structures transparent (constructible, inspectable, serializable by caller) | partial | Constructible/inspectable: **yes** — all twenty-one `Table` fields are `pub` (`table.rs:88-149`), `Player` is plain scalars, and `Table::nlh_primed` accepts an injected deck. Serializable: still **no** for `Table`/`Seats`/`Seat`/`Player`/`ForcedBets`/`Cards`; `BoxedCards(Box<[Card]>)` (`src/arrays/sliced.rs:24`) keeps a private field. Serializable where the wire needs it: `TableAction`, `PlayerState`, `GameType`, `GamePhase`, `BettingStructure`, `HandHistory`, `SessionView`. |
...
1. **Write `TableState` and `snapshot`/`restore`** — scoped as **[EPIC-88](epics/EPIC-88_Table_Snapshot.md)** on 2026-08-29. Moves **retention** 3 → 4 and closes the headline discontinuity.

   **Correction to this recommendation as first written.** The 0.10.0 run said "derive `Serialize`/`Deserialize` down the `Table` field tree." Scoping the EPIC surfaced a better-argued alternative already on record: `EPIC-37_Mobile_Engine.md:275-278` rejects serde on `Table` because deriving on the engine freezes its 21 public fields into the wire format — and, for EPIC-37's FFI, into the ABI. EPIC-88 therefore builds a `TableState` DTO instead, keeping `Table`'s layout free to change. Scoping also turned up two codec blockers neither document had: `Card`'s deserializer returns `Ok(0)` — a *blank card* — on a parse failure (`src/card.rs:379-389`), so a restore built on it could not refuse bad bytes; and `Cards::from_str("")` returns `Err` rather than an empty pile (`src/cards.rs:920-922`), so a pre-flop board cannot round-trip through a single-string field. Both are EPIC-88 Phase 0. *(This item also supersedes the 0.8.2 recommendation to write `TryFrom<&pkstate::PKState> for Table`, which is void — `pkstate` is no longer a dependency.)*
2. **Reconsider default features.** All eight are on, including `store`, which builds a bundled C SQLite for every default `cargo add pkcore`. Trimming the default set to the pure core plus `equity` (keeping `parallel`, which costs nothing off-wasm) would flip checklist item 8 to *pass*. Breaking for existing consumers → `/epic`.
3. **Widen the `Pile` combinatorics signatures** to `impl Iterator<Item = Vec<Card>>` (`src/lib.rs:937,942`), and mirror on `Cards`/`Deck`/`Outs`. RPITIT is available and `Pile` is never used as `dyn Pile`. Semver-breaking, but 13 internal call sites and **zero** across the 15 pkcore consumers. Moves checklist item 4 toward *pass*; does **not** reach coupling 5 on its own — `uuid` and `wincounter` would have to go too, and they are recorded as deliberate.
