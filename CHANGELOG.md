...
- **Aligned onto the single-engine `Table` of 0.8.0**
  ([EPIC-83](docs/epics/EPIC-83_Table_Decelled.md) merged into this branch).
  `TableOf<S>` absorbs the sixteen methods EPIC-83 ported off the retired
  `TableCelled` — `nlh_primed`, `act_new_hand`, `act_shuffle_deck`,
  `get_seat_handle`, `is_betting_started`, `determine_betting_phase`, the
  `eval_*` family and the `commentary_*` family. Only `nlh_primed` needed the
  `S::Sealed == Card` bound; the rest never touch the deck, so they stay
  scheme-agnostic.
- **`TryFrom<&Table>` is now `TryFrom<&TableOf<S>>`** for `Game`, `FlopEval`,
  `TurnEval` and `RiverEval`. All four delegate to `Table::build_game`, which
  reads the board and the seats and never the deck, so no bound is needed. The
  concrete `TryFrom<&Table>` keeps working through the alias.
- **`PartialEq` / `Eq` on `TableOf<S>` and `SealedDeck<S>` are hand-written**,
  for the reason `Clone` and `Debug` already were: a derive would add
  `S: PartialEq` on the *scheme*, which neither type stores. EPIC-83 derives
  both on `Table` because `Nubificus` compares through its table; this is the
  generic equivalent, field for field. Deck equality is order-sensitive —
  same payloads in the same slots in the same order.
- **`Default for TableOf<S>` replaces `Default for Table`**, bounded on
  `S::Sealed == Card`, because building a full 52-card deck means sealing one.
  `Table::default()` resolves through `NullSeal` and is unchanged.
- **BREAKING: `Table` is now `TableOf<S: CardSeal>`, and its deck is sealed**
  ...
  **always** sealed; the no-secrecy case is `NullSeal`, an identity scheme
  ...
  the invariant: **a deck you cannot read is a deck you cannot sort**. Eighteen
  `TableOf` methods and its `Default` carry that bound (the constructors, the
  dealing paths, and `reset` / `end_hand` / `abort_hand`); everything else
...
- **`SealedDeck::iter`** — the deck in dealing order, top card first, bounded
  on `S::Sealed == Card` alongside `sort_in_place` and `Display`. A deck you
  cannot read is a deck you cannot walk.
