...
impl TableCelled {
    ...
    #[must_use]
    pub fn generate_seats(count: u8, cards_per: u8) -> SeatsCell {
    ...
    #[must_use]
    pub fn nlh_primed(seats: SeatsCell, dealt: &CardsCell, forced_bets: ForcedBets) -> Self {
    ...
    #[must_use]
    pub fn nlh_from_seats(seats: SeatsCell, forced: ForcedBets) -> Self {
...
impl TryFrom<&Pluribus> for TableCelled {
    ...
    fn try_from(pluribus: &Pluribus) -> Result<Self, Self::Error> {
        let seats = SeatsCell::from(pluribus.players.clone());
        for seat in seats.borrow_all() {
...
mod casino__table_celled_tests {
    ...
    fn nlh_from_seats__not_holding() {
        let table = TableCelled::nlh_from_seats(SeatsCell::new(TestData::the_hand_players()), ForcedBets::new(50, 100));
        assert_eq!("No Limit Hold'em Table", table.name);
        assert_eq!(GameType::NoLimitHoldem, table.game);
        // assert_eq!(GamePhase::NewHand, table.phase.);
        assert_eq!(8, table.seats.size());
    ...
    #[test]
    fn bring_it_in() {
        let table = TableCelled::nlh_from_seats(SeatsCell::new(TestData::min_seats()), ForcedBets::new(50, 100));
    ...
    fn determine_street_equity_possible_with_different_chip_totals_returns_separate_sorted_entries() {
        ...
        let table = TableCelled::nlh_from_seats(SeatsCell::new(vec![seat_0, seat_1, seat_2]), ForcedBets::new(50, 100));
        let equity = table.determine_street_equity_possible();
