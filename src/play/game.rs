    ...
    pub fn river_display_results(&self) {
        match self.river_case_eval() {
            ...
            Ok(case_eval) => {
                ...
                println!(
                    "The River: {} {} {}",
                    self.board.flop, self.board.turn, self.board.river
                );
...
impl<S: CardSeal> TryFrom<&TableOf<S>> for Game {
    type Error = PKError;

    /// Delegates to [`TableOf::build_game`], which is the single definition of
    ...
    /// # Errors
    ///
    /// Propagates whatever [`TableOf::build_game`] returns — chiefly
    ...
    /// ```
    fn try_from(table: &TableOf<S>) -> Result<Self, Self::Error> {
        table.build_game()
    }
}
