...
| [EPIC-80](docs/EPIC-80_Kernel_Extraction.md) | Poker Evaluation Kernel Extraction (CKC) — move the Cactus Kev kernel (lookup tables, `Card`/`Rank`/`Suit`, `HandRank`, `Five`/`Six`/`Seven`) out of pkcore and down into `ckc-rs` 0.2.0: `no_std`, zero default dependencies, `standard52` namespace, C(52,5) golden-oracle differential test | **Phases 0–2, 4 Complete** — kernel extracted and proven on all 2,598,960 hands; Phase 3 (pkcore adapter layer) and Phase 5 (publish + downstream migration) Planned |
...
- **EPIC-80 through EPIC-89** — pkcore second block (the EPIC-00–39 block filled). Claimed 2026-07-25: `EPIC-80 Poker Evaluation Kernel Extraction` (move the Cactus Kev kernel down into `ckc-rs` 0.2.0 and depend on it as a crate). Next free pkcore number: `EPIC-81`. (`EPIC-79 Mental Poker` predates this block and stays put.)
