...
| [EPIC-39](docs/epics/EPIC-39_Decider_Range_Model.md) | Decider Opponent-Range Model — a game-state-derived `villain_range(state) -> Combos` (from position/action, never identity) fed to the equity engine's existing `PlayerSpec::Range`, unblocking the `outs` + `preflop_charts` knobs deferred in EPIC-36 (§5/§6); preserves the no-opponent-awareness invariant | **Complete** (`0.12.0`) |
| [EPIC-86](docs/epics/EPIC-86_Browser_Bindings.md) | Browser Bindings — `pkcore` in the browser via `wasm-bindgen`, shipped from `pkwasm` as the ESM npm package `@imperialbower/pkcore-wasm`; mirrors `pkcore.js`'s 22-class surface method-for-method so one mental model serves both bindings; object-scoped state (no `thread_local!` singletons) so a page can embed more than one table; chip amounts cross the boundary as validated `f64` because `wasm-bindgen` silently `ToUint32`-coerces integer params; contract-only here, implementation lands in `pkwasm` | Phase 0 spike landed (`pkwasm` @ `f5c029a`) — 64.7 KB gzipped, browser-verified; Phases 1–5 planned |
| [EPIC-87](docs/epics/EPIC-87_Pluribus_Export.md) | Pluribus-Format Hand Export — `Unumable`, the write half of `Plurable`; renderers for `Card`/`Two`/`Three`/`Four`/`Five`/`HoleCards`/`Board`; street-divider re-derivation; `TryFrom<&Table> for Pluribus` inverting the `DEFECT_021` cumulative-amount conversion; three round-trip tiers that turn the 10,000-hand `data/pluribus/raw` corpus into a regression suite for the replay engine; no new dependency, no new feature flag | **Complete** (`0.10.0`) — Tier 1 9,992/10,000, Tier 2 9,901/10,000 with zero unexplained, divider theory confirmed 10,000/10,000; three findings in the EPIC's corrigendum, including an all-in run-out gap in `Table` filed as `DEFECT_025` |
...
- **EPIC-70 through EPIC-78** — pkdealer second downstream block (the EPIC-40 block filled). Claimed 2026-07-20: `EPIC-70 Collusion & Cheat Detection` (colluding bots + live `pkdealer_sentinel` detector). Next free pkdealer number: `EPIC-71`. (`EPIC-79 Mental Poker` predates this block as a pkcore spike/special and stays put — it is the *fix* for the spectator-token hole EPIC-70 exploits.)
- **EPIC-80 through EPIC-89** — pkcore second internal block, opened once the pkcore run reached `EPIC-39`, the top of its first block. **Retroactively documented 2026-08-29:** the block was in use for a month before it was written down here, which is why the EPICs below were never listed in this policy. In use: `EPIC-81 pkcore on the ckc-rs Kernel`, `EPIC-82 The Betting Kernel` (drafted on `origin/EPIC-79b` @ `6e2aae8`, **never merged to `main`** — reserve the number anyway), `EPIC-83 Table Decelled`, `EPIC-84 Sealed Table via the cardpack Seal Kernel`, `EPIC-85 JavaScript Bindings`, `EPIC-86 Browser Bindings`, `EPIC-87 Pluribus-Format Hand Export`, `EPIC-88 Table Snapshot & Restore` (claimed 2026-08-29 — lifts `EPIC-37` Phase 3 out of the mobile epic because the capability also serves the pkdealer resumable table service and `EPIC-82`'s `apply(state, action)`). `EPIC-80` was never used. Next free pkcore number: `EPIC-89`. (`docs/epics/EPIC-81_Ckc_Rs_Dependency.md:532` still reads *"Next free: `EPIC-82`"* — stale, superseded by this line. The `EPIC-95`–`EPIC-99` and `EPIC-999` band is pkcore meta — Distinct, The Answers, Philosophy, Glossary, References, ramblings — and is not an allocatable block.)
