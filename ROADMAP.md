...
| [EPIC-87](docs/epics/EPIC-87_Pluribus_Export.md) | Pluribus-Format Hand Export — `Unumable`, the write half of `Plurable`; renderers for `Card`/`Two`/`Three`/`Four`/`Five`/`HoleCards`/`Board`; street-divider re-derivation; `TryFrom<&Table> for Pluribus` inverting the `DEFECT_021` cumulative-amount conversion; three round-trip tiers that turn the 10,000-hand `data/pluribus/raw` corpus into a regression suite for the replay engine; no new dependency, no new feature flag | Planned |
